...
例如某工作流若与 12 kHz 解码、48 kHz TX、DSP Worker、rigctld owner 或控制租约冲突，约束永远优先；
...
|----------|------------------|-------------------|
| 创意/需求 | `brainstorming` | `brief --task`; spec 引用相关 AD/NFR/§；先查 §3.2/3.3 与 I8–I11 |
| 计划 | `writing-plans` / `create-plan` | `brief <files>`;计划任务清单必须包含受影响 SDD 章节 + 一条 doc-sync 任务;`trace` 验证引用覆盖 |
| 隔离 | `using-git-worktrees` | 基线验证 = `pytest` + `check` |
...
| 验证 | `verification-before-completion` | 完成门禁 = `pytest` + `check --staged` + doc-sync 全绿 |
