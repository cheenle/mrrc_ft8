subroutine ft8apsetvar(lmycallstd,lhiscallstd,numthreads)
  ...
  ! Headless requests do not retain AP masks from prior contexts.
  ...
  if(.true.) then ! rebuild AP state for every headless request
...
! shall hash both callsigns for making AP masks with nonstandard callsign message
    ...
    if(.true.) then ! rebuild grid- and callsign-dependent masks
      hiscallprev=hiscall
