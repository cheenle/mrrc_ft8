...
function serverMessageToRow(
	m: ServerDecodeMessage,
	slotId: number,
): FT8DecodedMessage {
	return {
...
function rowToCandidate(row: FT8DecodedMessage): DecodeCandidate | null {
	const call = (
		row.call ??
		extractTransmitterCallsign(row.message) ??
		""
	).toUpperCase();
	if (!call) return null;
	return {
...
const SEQUENCER_STATE_LABELS: Record<string, string> = {
	idle: "IDLE",
	calling: "CQ_SENDING",
	replying: "REPLY_SENDING",
	report: "SENDING_REPORT",
	roger_report: "SENDING_R_REPORT",
	rogers: "SENDING_RR73",
	signoff: "SENDING_73",
	done: "IDLE",
};
...
async function sampleClockOffset(): Promise<{
	offsetMs: number;
	...
	return {
		offsetMs: serverMs + 500 - (tx + rtt / 2),
		uncertaintyMs: rtt / 2 + 500,
	};
}
...
async function checkClock(): Promise<ClockVerdict> {
	...
	if (drift < 2000) {
		return {
			status: "warn",
			offsetMs: best.offsetMs,
			message: `Off ${offsetStr} \u2014 watch it`,
		};
	}
	return {
		status: "bad",
		offsetMs: best.offsetMs,
		message: `Off ${offsetStr} \u2014 fix device clock`,
	};
}
...
export default function App() {
	...
	const [clockVerdict, setClockVerdict] = useState<ClockVerdict>({
		status: "unknown",
		offsetMs: 0,
		message: "Checking\u2026",
	});
	...
	const [myCall, setMyCall] = useState<string>(
		() => localStorage.getItem("ft8_myCall") || "N0TMP",
	);
	const [myGrid, setMyGrid] = useState<string>(
		() => localStorage.getItem("ft8_myGrid") || "EM12",
	);
	...
	const [finalMessageMode, setFinalMessageMode] = useState<"RR73" | "RRR">(
		() => {
			return (
				(localStorage.getItem("ft8_finalMessageMode") as "RR73" | "RRR") ||
				"RR73"
			);
		},
	);
	...
	const [deviceData, setDeviceData] = useState<Record<string, any> | null>(
		null,
	);
	...
	const isReturningUserRef = useRef(
		typeof localStorage !== "undefined" &&
			Object.keys(localStorage).some(
				(k) => k.startsWith("ft8_") && k !== "ft8_lastSeenUpdate",
			),
	);
	...
	useEffect(() => {
		...
		for (const batch of lastDecodes) {
			...
			for (const m of batch.messages)
				rows.push(serverMessageToRow(m, batch.slot_id));
		}
	...
	useEffect(() => {
		...
		for (const batch of lastDecodes) {
			...
			for (const m of batch.messages) {
				const fromSelected =
					selectedCall !== "" && m.call.toUpperCase() === selectedCall;
	...
	const [selectedCandidate, setSelectedCandidate] =
		useState<DecodeCandidate | null>(null);
	...
	const [fsmQueue, setFsmQueue] = useState<
		{ callsign: string; report?: string | null; distance?: number }[]
	>([]);
	...
	const handleReplyRow = useCallback(
		async (row: FT8DecodedMessage) => {
			const candidate = rowToCandidate(row);
			if (!candidate) return;
			setTargetCall(candidate.call);
			...
			if (!res.ok) {
				setTxNotice(
					res.reason === "lease_required"
						? "Control is held by another session"
						: `Reply rejected: ${res.reason ?? res.status}`,
				);
				return;
			}
			const scheduled = res.body?.scheduled_tx as
				| { utc?: string; deferred?: boolean }
				| undefined;
			if (scheduled?.deferred && scheduled.utc) {
				setTxNotice(
					`Reply armed → TX at ${scheduled.utc.slice(0, 2)}:${scheduled.utc.slice(2, 4)}:${scheduled.utc.slice(4, 6)} UTC`,
				);
			}
		},
		[ensureLease],
	...
	const openSettingsModal = useCallback(async () => {
		...
		setDecoderThreads(
			typeof s.decoder_threads === "number" ? s.decoder_threads : 0,
		);
		if (typeof s.auto_call_new_dxcc === "boolean")
			setAutoSequence(s.auto_call_new_dxcc);
		if (typeof s.auto_band_hunt === "boolean")
			setAutoBandHunt(s.auto_band_hunt);
	...
	const saveSettings = useCallback(async () => {
		...
		if (baseline.auto_call_new_dxcc !== autoSequence)
			changes.auto_call_new_dxcc = autoSequence;
		if (baseline.auto_band_hunt !== autoBandHunt)
			changes.auto_band_hunt = autoBandHunt;
	...
	const saveDeviceConfig = useCallback(
		async (form: DeviceForm): Promise<string | null> => {
			setDeviceSaving(true);
			try {
				const res = await mrrc.saveDevices(
	...
	return (
		<div className="min-h-screen bg-app text-text-main font-sans flex flex-col p-4 select-none">
				...
				<div className="flex items-center gap-6 flex-wrap">
					<div className="flex flex-col">
						...
						<div className="flex gap-2">
							...
							>
								<div
									className={`w-2 h-2 rounded-full ${audioActive ? "bg-green-600 dark:bg-[#4caf50] shadow-[0_0_8px_#4caf50]" : "bg-[#2a2c31]"}`}
								></div>
                  ...
                  control lease, OBSERVER otherwise. */}
							<span
								...
								title={
									snapshot.lease.mine
										? "This session holds radio control"
										: "No radio control (observer)"
								}
							>
								<div
									className={`w-2 h-2 rounded-full ${snapshot.lease.mine ? "bg-green-600 dark:bg-[#4caf50] shadow-[0_0_8px_#4caf50]" : "bg-[#2a2c31]"}`}
								></div>
              ...
              safety.faults includes the "audio" Interlock value). */}
					<div className="flex flex-col">
						...
						<span
							...
							title={
								audioFault
									? "Server audio interlock FAULT"
									: "Server audio interlock OK"
							}
						>
							<div
								className={`w-2 h-2 rounded-full ${audioFault ? "bg-red-500 shadow-[0_0_8px_#ef4444] animate-pulse" : "bg-green-600 dark:bg-[#4caf50] shadow-[0_0_8px_#4caf50]"}`}
							></div>
				...
				<div className="flex flex-col items-end min-w-[120px]">
					...
					<div
						...
						title={clockVerdict.message}
					>
						<span
							className={`h-2 w-2 rounded-full transition-colors duration-500 ${
								clockVerdict.status === "ok"
									? "bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.7)]"
									: clockVerdict.status === "warn"
										? "bg-yellow-400 shadow-[0_0_6px_rgba(250,204,21,0.7)] animate-pulse"
										: clockVerdict.status === "bad"
			...
			<div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-[400px]">
					...
					<div className="bg-panel border border-border-subtle rounded-lg flex flex-col h-[300px] max-h-[300px] shrink-0 overflow-hidden">
						<div className="bg-header border-b border-border-subtle px-3 py-2 flex justify-between items-center rounded-t-lg shrink-0">
							<h3 className="text-[11px] font-bold text-text-muted tracking-widest uppercase flex items-center gap-2">
								<Activity
									size={14}
									className="text-green-600 dark:text-[#4caf50]"
								/>
								Band Activity
							</h3>
							<button
							...
							<div className="flex-1 overflow-y-auto space-y-0.5 pr-1">
								{rxLog.length === 0 && (
									<div className="text-text-muted flex items-center justify-center h-full opacity-50 p-6 text-center text-xs">
										Awaiting FT8 signals...
										<br />
										(Audio decoded every 15s synced period)
									</div>
								)}
								{rxLog.map((log, i) => {
									if (log.isDivider) {
										return (
											<div
												key={i}
												className="flex items-center justify-center py-2 opacity-50"
											>
												<span className="text-[9px] font-mono tracking-widest text-text-muted">
													{log.message}
												</span>
											</div>
										);
									...
									return (
										<div
											...
											onClick={() => {
												...
												const callerPeriod =
													log.periodIndex !== undefined
														? log.periodIndex
														: (() => {
																const seconds = parseInt(
																	log.time.substring(4, 6),
																	10,
																);
										...
										>
											...
											<span
												className={
													log.snr > -10 ? "text-green-400" : "text-red-400"
												}
											>
												{log.snr}
											</span>
											...
											<span className="text-text-main group-hover:text-text-highlight font-bold flex items-center flex-wrap">
												...
												<DxccBadges
													call={log.call}
													isNewDxcc={log.isNewDxcc}
													workedCalls={snapshot.station.worked_calls}
												/>
											</span>
										</div>
									);
													...
													const callerPeriod =
														log.periodIndex !== undefined
															...
															: log.time && log.time.length >= 6
																...
																: null;
													if (callerPeriod !== null)
														setTxPeriod(callerPeriod === 0 ? 1 : 0);
												}
											}}
											onDoubleClick={() => {
												if (!log.isTx) void handleReplyRow(log);
											}}
											className="grid grid-cols-[55px_40px_60px_1fr] gap-2 hover:bg-btn cursor-pointer p-1 rounded transition-colors group text-[11px] items-center"
										>
											<span className="text-zinc-500">{log.time}</span>
											...
											<span className="text-blue-400">{log.freq}Hz</span>
											<span
												className={`group-hover:text-text-highlight ${textClass} flex items-center flex-wrap`}
											>
												{log.message}
												<DxccBadges
													call={log.call}
				...
				<section
					className={`lg:col-span-7 border border-border-subtle rounded-lg overflow-hidden flex flex-col relative h-[300px] lg:h-auto ${theme === "dark" ? "bg-[#050505]" : "bg-white"}`}
				>
					<div className="absolute top-0 inset-x-0 bg-black/40 backdrop-blur-sm px-3 py-1 border-b border-border-subtle flex justify-between z-10 pointer-events-none">
						<span
							className={`text-[9px] font-mono tracking-tighter ${theme === "dark" ? "text-[#4caf50]" : "text-green-600"}`}
						>
							WATERFALL (200 - 3000 Hz)
						</span>
			...
			<footer className="bg-header border border-border-subtle rounded-lg p-4 shadow-inner mt-3">
				<div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
					<div className="w-full lg:w-auto space-y-3 lg:border-r border-border-subtle lg:pr-6 shrink-0">
						<div className="flex flex-col">
							<label className="text-[9px] uppercase tracking-widest text-text-muted mb-1">
								My Station
							</label>
							<div
							...
							>
								<span
									className={`border border-border-subtle rounded px-3 py-1.5 text-xs font-mono uppercase font-bold min-w-[80px] text-center ${
										theme === "dark"
											? "bg-[#050505] text-[#4caf50]"
											: "bg-white text-green-600"
									}`}
									title="Station identity (managed on the station)"
								>
									{myCall}
								</span>
								<span
									className={`border border-border-subtle rounded px-3 py-1.5 text-xs font-mono uppercase min-w-[60px] text-center ${
										theme === "dark"
											? "bg-[#050505] text-text-muted"
								...
								<span
									className={`border border-border-subtle rounded px-3 py-1.5 text-xs font-mono min-w-[80px] text-center ${
										theme === "dark"
											? "bg-[#050505] text-text-main"
						...
						<div className="flex flex-col">
							...
							<input
								...
								onChange={(e) => {
									...
									setTargetCall(e.target.value.toUpperCase());
								}}
								className="bg-app border border-border-input rounded px-2 py-1 text-xs font-mono w-full max-w-[200px] focus:outline-none focus:border-blue-500 text-text-main uppercase"
							/>
						</div>
					</div>
					...
					<div className="w-full lg:w-auto flex-1 grid grid-cols-2 gap-2 px-0 lg:px-4">
						...
						{(isTransmitting || isTxQueued) && (
							<div
								className={`col-span-2 mt-1 flex items-center justify-center gap-2 p-2 ${isTransmitting ? "bg-rose-950/40 text-rose-400 border-rose-900 animate-pulse" : "bg-amber-950/40 text-amber-500 border-amber-900"} border rounded font-bold text-[10px] uppercase tracking-widest`}
							>
								<Activity
									size={12}
									className={isTransmitting ? "" : "opacity-50"}
								/>{" "}
								{isTransmitting ? "Transmitting..." : "TX Queued..."}
							</div>
						)}
					</div>
							...
							{autoSequence && fsmQueue.length > 0 && (
								<div className="w-full lg:w-32 mt-1 max-h-[72px] overflow-y-auto flex flex-col gap-0.5 custom-scrollbar">
									{fsmQueue.map((c) => (
										<div
											key={c.callsign}
											className="flex items-center justify-between px-1.5 py-0.5 rounded bg-black/30 border border-border-subtle/40"
										>
											<span className="text-[9px] font-mono font-bold text-sky-400 tracking-wide">
												{c.callsign}
											</span>
						...
						<div className="flex flex-col items-center justify-center">
							...
							>
								<span className="text-[10px] font-bold tracking-widest uppercase">
									{txEnabled ? "TX Enabled" : "Enable TX"}
								</span>
								<div
									className={`w-8 h-2 rounded-full relative ${
										txEnabled
											? theme === "dark"
												? "bg-[#4a1a1a]"
												: "bg-red-200"
								...
								>
									<div
										className={`absolute left-0 top-0 w-3 h-2 rounded-full transition-all ${
											txEnabled
												? theme === "dark"
				...
				<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
                  ...
                  Callsign/grid are configured on the station (server). */}
							<div className="flex flex-col gap-1">
								...
								<span className="bg-app border border-border-input rounded px-3 py-2 text-sm font-mono w-full text-text-main uppercase">
									{myCall}
								</span>
							</div>
							<div className="flex flex-col gap-1">
							...
							<div className="flex flex-col gap-3 pt-2 border-t border-border-subtle">
								...
								<div className="flex flex-col gap-1">
									...
									>
										...
										<option value="3">
											3 - Max (Slowest, Decodes weak signals)
										</option>
									</select>
								</div>
								<div className="flex flex-col gap-1">
							...
							<div className="flex flex-col gap-2 pt-2 border-t border-border-subtle">
								...
								<div className="flex items-center justify-between">
									<div className="flex flex-col">
										...
										<span className="text-[9px] text-text-muted">
											Auto-call a not-yet-worked DXCC entity
										</span>
									</div>
									<button
								...
								<div className="flex items-center justify-between">
									<div className="flex flex-col">
										...
										<span className="text-[9px] text-text-muted">
											Hunt new DXCC entities across bands
										</span>
									</div>
									<button
								...
								<div className="flex flex-col gap-1 mt-1">
									...
									<select
										...
										onChange={(e) =>
											setFinalMessageMode(e.target.value as "RR73" | "RRR")
										}
										className="bg-app border border-border-input text-text-main rounded px-3 py-2 text-xs font-mono w-full focus:outline-none focus:border-[#4caf50]"
									>
                  ...
                  are all managed on the station. */}
							<div className="pt-3 border-t border-border-subtle text-[10px] text-text-muted leading-relaxed">
								<span className="font-bold uppercase tracking-widest text-text-main">
									Server
								</span>{" "}
								— Radio/CAT, audio input, and external logging integrations are
								managed on the station. This client only controls the FT8
								session.
							</div>
						</div>
				...
				<div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
					<div className="bg-panel border border-border-subtle p-6 rounded-lg shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
						...
						<div className="space-y-4 text-xs text-text-main leading-relaxed">
							<p>
								Welcome to the MRRC-FT8 Desktop client! This is a browser-based
								control panel for the FT8 station. All DSP decoding, radio
								control, and audio I/O run on the station server — this client
								connects to it over the network.
							</p>

							<div>
								<h3 className="font-bold text-green-600 dark:text-[#4caf50] mb-1">
									1. Radio & Audio
								</h3>
								<p>
									Radio/CAT control, audio input and output, and the FT8 DSP
									decoder are all managed on the station. This client needs no
									microphone or sound-card access in the browser.
								</p>
							</div>
							...
							<div>
								...
								<p>
									Your Call Sign, Grid Square, and CAT control are configured on
									the station and read from the server. The{" "}
									<strong>Settings</strong> modal adjusts decode and
									QSO-behaviour preferences.
								</p>
							</div>
							...
							<div>
								...
								<p>
									Select your band using the pill buttons (FT8 mode only; the
									server tunes the rig). FT8 decodes at :00, :15, :30, :45.
								</p>
								<p>
									FT8 relies strictly on synchronized UTC time — verify your
									system clock is accurate.
								</p>
								<p>
									Each decode carries a DXCC badge from the server:{" "}
									<strong>N</strong> (not-yet-worked entity), <strong>W</strong>{" "}
									(already-worked entity), or <strong>B4</strong> (base callsign
									already in the log).
								</p>
								<p>
									Enable TX and the server sequencer automatically manages CQ,
									grid exchange, SNR report, and 73.
								</p>
							</div>
							...
							<div>
								...
								<p>
									Completed QSOs are recorded by the station server. External
									logging integrations (Wavelog, Cloudlog, etc.) are configured
									on the station, not in this client.
								</p>
							</div>

							<div className="pt-4 mt-4 border-t border-border-subtle">
								<h3 className="font-bold text-green-600 dark:text-[#4caf50] mb-1">
									Tested Radios & Feedback
								</h3>
								<p className="mb-2">
									<strong>Tested:</strong> IC-705, IC-7300, Yaesu FTX-1, Flex
									6400, Flex 8400. FT-817 in progress.
								</p>
								<p className="mb-2">
									If you have success with your radio model — or hit a protocol
							...
							<div className="pt-4 mt-4 border-t border-border-subtle">
								...
								<p className="mb-2">
									Created by <strong>Ondřej Koloničný, OK1CDJ</strong>.
								</p>
								<p className="mb-2">
									This project is open-source and licensed under the{" "}
									<strong>GNU General Public License v3 (GPL v3)</strong>.
								</p>
								<h4 className="font-bold text-text-main mt-4 mb-2">
									Acknowledgments
								</h4>
								<ul className="list-disc pl-5 mb-4 space-y-1 text-[11px] text-text-muted">
									<li>
										...
										FT8 and FT4 are digital amateur radio modes designed for
										weak-signal communication, originally developed by{" "}
										<strong>Joe Taylor (K1JT)</strong> and{" "}
										<strong>Steve Franke (K9AN)</strong> as part of the WSJT-X
										suite.
									</li>
									<li>
										<strong className="text-text-main">
										...
										FT8 decoding is performed by the station server, not in the
										browser.
									</li>
								</ul>
							</div>
						...
						<div className="mt-4 pt-3 border-t border-border-subtle/40">
							<p className="text-[9px] text-text-muted leading-relaxed">
								...
								programs, and protocol specifications for the modes FSK441,
								FST4, FST4W, FT4, FT8, JT4, JT6M, JT9, JT44, JT65, JTMS, Q65,
								QRA64, ISCAT, and MSK144 are Copyright © 2001-2026 by one or
								more of the following authors: Joseph Taylor, K1JT; Bill
								Somerville, G4WJS; Steven Franke, K9AN; Nico Palermo, IV3NWV;
								Uwe Risse, DG2YCB; Brian Moran, N9ADG; John Nelson, G4KLA;
								Charles Suckling, DL3WDG; Roger Rehr, W3SZ; Greg Beam, KI7MT;
								Michael Black, W9MDB; Edson Pereira, PY2SDR; Philip Karn, KA9Q;
