...
## AD-016 — RUMLogNG bidirectional QSO sync (AppleScript push + read-only Core Data poll)
...
- 跨库去重：rumlog_uuid 精确匹配优先；否则 dx_call+band 相同且完成时刻差 ≤120 s；拉取确认时确认窗口内**全部**重复行（否则重复历史无限 requeue）。推送前按同一窗口去重（历史重复行只推一条），并先检查该 QSO 是否已有**已确认兄弟行**（`pushed_to_rumlog=1`）——已有则待推送副本直接确认、不推送，防止 JTDX 重导入副本再次推入 RUMLogNG 造成重复。JTDX 导入侧在精确键 `(dx_call, 开始日期, started_utc, band)` 之外按同一完成时刻窗口跨源去重（RUMLogNG 拉取行 `started_utc` 由完成时刻派生，与 ADIF `TIME_ON` 相差可达约 1 分钟，精确键认不出同一 QSO）。
