...
第二波清理（同日稍后，用户现场发现"同呼号同信息不同日期"）：requeue 风暴期（08-11~21，RUMLogNG 离线致 42 次重推）部分推送行以**墙钟时间**落入 RUMLogNG，日期偏离真实 QSO 数天、±120 s 窗口认不出，又被拉回本地成 `source='rumlog'` 副本。按精确谓词（本地 source='rumlog' 行与一天以上前的 jtdx/live 行在呼号/波段/模式/双方 RST/grid 全同）锁定 46 对，经本地行携带的 rumlog_uuid 1:1 匹配 RUMLogNG 侧（46/46 命中），两边各删 46 行（RUMLogNG 11649→11603，本地 11546→11500）；备份 /tmp/mrrc-dedup-backup-20260822/（CoreQsoModel_1-wave2.sqlite 快照校验通过 + mrrc-ft8-wave2.db），RUMLogNG 退出后操作、事后重启。另查明 BG1SUD 等"同呼号多日期 SSB 行"为真实通联、仅 note 被 RUMLogNG 站史自动填充污染，保留不删。
...
- **功能**：设备配置可配置化——HAM rig 型号/串口/波特率/rigctld 端口/音频设备的集中配置，支持桌面客户端与 PWA 的 Settings UI 修改，保存后应用重启生效。
- **新模块**：`server/engine/device_config.py` — `data/device-config.json` 读写（原子写 tempfile+rename）、`effective_config`（file > env > default 优先级）、`merge_into`（server 启动时覆盖 audio_device + rigctld_port）、`enumerate_audio_devices`/`enumerate_serial_devices` 硬件枚举、`validate` 校验、`source_of` 来源追踪、`shell_env_lines`（restart.sh 用）、`DeviceConfigStore`（spawn_restart → detached restart.sh）。
- **API**：`GET/PUT /api/v1/devices`（require_session）+ `POST /api/v1/devices/apply`（require_session；TX active → 409 tx_active；apply 返回 202 restarting；重启期间 client 断连 ~20 s 后重新登录）。
- **Deploy**：`restart.sh` 启动前读 `data/device-config.json`（存在时覆盖 `MRRC_FT8_RIG_MODEL`/`DEVICE`/`BAUD`/`RIGCTLD_PORT`）；env 回退保持向后兼容。
- **桌面客户端**：Settings 弹窗新增 Devices 分区（型号下拉 4 种 hamlib 模型含自定义/串口路径/波特率/rigctld 端口/音频设备选择）；保存 + 应用重启按钮。
- **PWA**：☰ 抽屉新增 Devices 标签页（同款字段，serial manual input path + custom model fallback）。
