...
| NFR-088 | Propagation-driven band hunt | With setting `auto_band_hunt` enabled and `MRRC_FT8_BAND_HUNT_URL` configured, the server polls an external `/api/band_hunt` endpoint (HTTP-only cross-repo boundary) for FT8 bands that nearby grids (within radius_km of our grid) are actively hearing; when idle it tunes the rig to the top band that still has unworked DXCC entities, then auto-call (NFR-087) closes the QSO. Guards: idle-only, min-dwell cooldown, never tunes during TX, endpoint failure skips the round |
